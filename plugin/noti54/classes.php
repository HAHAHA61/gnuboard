...
class G5_NOTI_54 {
	...
	public function add_hooks(){
		...
		add_event('write_update', array($this, 'write'), 1, 5);        
	...
	public function write_insert($board, $wr_id, $w, $qstr, $redirect_url) {
		...
		if ($w == '') {
			...
			// 관리자에게 알림 보내기
			$admin_id = 'admin';  // 관리자의 회원 아이디 지정
			$admin_member = get_member($admin_id, '*', true);
	
			if ($admin_member && $admin_member['mb_is_noti']) {
				$ph_to_case = 'admin';
				$ph_from_case = 'new_post';
				$tmp_url = "/".G5_BBS_DIR."/board.php?bo_table=".$board['bo_table']."&wr_id=".$wr_id;
				$tmp_wr_subject = cut_str(strip_tags($wr['wr_subject']), 90);
	
				$sql = "INSERT INTO ".$this->db_table." SET ph_to_case = '".$ph_to_case."', ph_from_case = '".$ph_from_case."', bo_table = '".$board['bo_table']."', rel_bo_table = '".$board['bo_table']."', wr_id = '{$wr['wr_id']}', rel_wr_id = '0', mb_id = '{$admin_member['mb_id']}', rel_mb_id = '', rel_mb_nick = '', rel_msg = '".sql_real_escape_string(cut_str(strip_tags($wr['wr_content']), 70))."', parent_subject = '".sql_real_escape_string($tmp_wr_subject)."', rel_url = '".$tmp_url."', ph_readed = 'N', ph_datetime = '".G5_TIME_YMDHIS."', wr_parent = '{$wr['wr_id']}'";
				$result = sql_query($sql, false);
				...
				$this->mb_noti_count_update($admin_member['mb_id']);
			}
		}
	}
